...
namespace ZG
{
    public class InstanceAuthoring : MonoBehaviour
    {
        class Baker : Baker<InstanceAuthoring>
        {
            ...
            {
                UnityEngine.Assertions.Assert.AreEqual(1, authoring.GetComponents<InstanceAuthoring>().Length,
                    authoring.name);
                ...
                Instance instance;
                instance.name = string.IsNullOrEmpty(authoring._nameOverride)
                    ? authoring.name
                    : authoring._nameOverride;
        ...
        [SerializeField] internal string _nameOverride;
